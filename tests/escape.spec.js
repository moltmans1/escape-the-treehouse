...
test.describe('Escape the Treehouse E2E Tests', () => {
  ...
  test('Test Case 4: Final Escape', async ({ page }) => {
    ...
    // 5. Click the Zipline hotspot (510, 270)
    await page.locator('canvas').click({ position: { x: 510, y: 270 } });
    dialogText = await page.evaluate(() => window.__gameState.dialogText);
    ...
    // 6. Click the pinned note on the wall (860, 200)
    await page.locator('canvas').click({ position: { x: 860, y: 200 } });
    ...
    // 9. Click the door on the balcony (930, 220) to go back inside
    await page.locator('canvas').click({ position: { x: 930, y: 220 } });
    await page.waitForFunction(() => window.__gameState.currentView === 'south');
  });
